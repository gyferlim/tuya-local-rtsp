...
echo 
print_color "$GREEN" "Checking if Go installed" && echo
# Check Go installation
if ! command -v go >/dev/null 2>&1; then
    print_color "$RED" "Error: " && echo "Go compiler is not installed or not found in PATH." && echo 
    print_color "$YELLOW" "Please install Go before running this build script." && echo 
    print_color "$CYAN" "For Debian/Ubuntu:  apt-get install golang" && echo && echo
    exit 1
fi
print_color "$GREEN" "✓ found: $(go version)" && echo
...
print_section "Authentication Commands" && echo
...
print_section "Camera Management Commands" && echo
...
print_section "RTSP Server Commands" && echo
...
print_section "Available Regions" && echo
...
print_section "Authentication Methods" && echo
...
print_color $PURPLE "Authentication Examples:" && echo
...
print_color $PURPLE "Camera Management Examples:" && echo
...
print_color $PURPLE "RTSP Server Examples:" && echo
...
echo
print_color $RED "Common Issues & Solutions:"  && echo
print_color "$RED" "❌" && print_color $YELLOW " Authentication fails:" && echo
...
print_color "$RED" "❌" && print_color $YELLOW " No cameras found:" && echo
...
print_color "$RED" "❌" && print_color $YELLOW " RTSP stream not working:" && echo
...
print_color "$RED" "❌" && print_color $YELLOW " Country code not found during password login:" && echo
...
print_color $GREEN "✅ For more help, run any command with --help" && echo
print_color $GREEN "✅ Example: ./tuya-ipc-terminal auth --help" && echo
